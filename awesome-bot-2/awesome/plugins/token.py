...
# 获取token  
async def getToken(qqNum) -> (str,int):
    ...
    if (useNum is not None and useNum > 0 ) or qqNum in myQQs:
        ...
        if resp.status_code == 200:
            res =  "点击该链接，输入您的学号和身份证后六位登录即可。请用浏览器打开该链接。"\
            +"http://raven520.top/sport?token="+resp.text+" \n温馨提示：直接点击该私密链接不需要您输入密令\
            （需浏览器里打开！），系统会帮你填好密令。如果提示服务链接失败，请您刷新页面！，ps:直接登录该网站\
            即可满分，无需校园网。"
            # 如果获取的QQ不是我的QQ
